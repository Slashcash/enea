...
std::pair<std::optional<RomSource::Error>, nlohmann::json> RomSource::readCache(const std::filesystem::path& path) const
{
    ...
    // If the cache is generated by and older version of the program we deem it invalid
    if (auto cacheVersion = result.find("version"); cacheVersion == result.end())
    {
        return std::make_pair(Error::INVALID_CACHE_FILE_VERSION, nlohmann::json());
    }
    else if (*cacheVersion != projectVersion)
    {
        return std::make_pair(Error::INVALID_CACHE_FILE_VERSION, nlohmann::json());
    }
