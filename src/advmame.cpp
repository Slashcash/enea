...
Result AdvMAME::runEffectively(const fs::path& theRomPath, const fs::path& theBasePath) const {
    //we build the command
    std::string console_command = getPath().string() + " -dir_rom " + theRomPath.parent_path().string() + ":" + theBasePath.string() + " -cfg " + configuration_file_path.string() + " " + theRomPath.stem().string();
